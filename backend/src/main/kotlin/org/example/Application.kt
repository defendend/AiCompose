...
fun Application.initializeDocsIndex() {
    ...
    try {
        ...
        // Пробуем загрузить метаданные существующего индекса
        val loaded = docsIndex.loadMetadata()

        // Всегда переиндексируем (SimpleEmbeddings не сохраняется в файл)
        // Но проверяем needsReindex() чтобы знать, изменились ли файлы
        val needsReindex = !loaded || docsIndex.needsReindex()

        logger.info(if (needsReindex) "🔄 Индексация документации..." else "🔄 Восстановление индекса документации...")
        val result = docsIndex.indexProjectDocs()

        if (result.success) {
            if (needsReindex) {
                // Сохраняем индекс только если файлы изменились
                docsIndex.save()
            }
